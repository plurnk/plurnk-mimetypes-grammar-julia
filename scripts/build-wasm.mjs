#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-julia.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-julia-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-julia.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as julia.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "julia.wasm"));
const bytes = (await readFile(path.join(repoRoot, "julia.wasm"))).length;
console.log(`julia.wasm: ${bytes} bytes (built from ${pin})`);
