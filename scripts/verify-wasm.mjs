#!/usr/bin/env node
// CI byte-identical reproducibility check for julia.wasm.
...
const committed = await readFile(path.join(repoRoot, "julia.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed julia.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-julia-verify-"));
...
console.log(`rebuilt julia.wasm sha256: ${rebuiltHash}`);
